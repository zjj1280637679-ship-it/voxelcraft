...
>
> 命名:玩家可见名为**《永远的山根乡》**(页面标题、菜单与全部 UI 文案);**VoxelCraft** 仅作为英文代号保留——仓库/包/文件名、`[vc]` 日志前缀与构建产物(如 `VoxelCraft.apk`),不出现在玩家可见文案中。
>
> 本文面向第一次接触本项目的读者。所有协议字段、错误码与数值上限均核对自仓库源码(出处标注在各节中);架构契约的原始定义见 `DESIGN.md`(当前为 v3:六页菜单、皮肤协议 v2、世界本地存档)。

---

## 1. 愿景与设计目标

《永远的山根乡》要解决的第一需求很朴素:**一台电脑、几部手机,连同一个 Wi-Fi,打开浏览器就能进同一个房间一起玩**。PC、安卓、iPhone 全部通过浏览器参与,无需安装(安卓另有可选的 App 壳,见第 9 章)。
...
|---|---|---|
| `{t:'hello', name, skin}` | `skin = {b, t, p, k}`(皮肤协议 v2,见 6.2) | 收到 `accepted` 后立即发送。字段示例:`{"t":"hello","name":"阿明","skin":{"b":5,"t":2,"p":6,"k":3}}`。房主校验:`name` 必须是字符串,trim 后截 16 字符,空则置 `'玩家'`;`skin` 走共享的 v2 校验/迁移规则——v2 形状(`b,t,p,k` 全整数且 `b,t,p ∈ 0..7`、`k ∈ 0..5`)→ 原样收下(只拷贝这四个键);**legacy `{s,p}`**(两者均为 `0..7` 整数)→ 迁移为 `{b:1, t:s, p:p, k:1}`;其余一切 → 坍缩为 `{b:1, t:0, p:0, k:1}`。重复 hello → 仅重发 `joined`,绝不重播 `pjoin`(迁移后的 re-hello 依赖此幂等性,见第 7 章) |
...
| 消息 | 字段 | 语义 |
|---|---|---|
| `{t:'joined', room, seed, id, players, edits}` | `players:[{id,name,p,ry,skin:{b,t,p,k}}]`,`edits:[[x,y,z,id],…]` | 对 `hello` 的单播应答。`id` 为收件人自己的中继 id;`players` 包含房主本人(取自实时 `playerRef`)与其他已 hello 成员、不含收件人,其中每个 `skin` 都是房主校验后的 v2 对象,字段示例:`players:[{"id":1,"name":"山根乡长","p":[8,40,8],"ry":0,"skin":{"b":1,"t":0,"p":0,"k":1}}]`;`edits` 序列化自房主的 `world.edits` 全量修改集 |
...
**legacy 兼容**:线上仍可能出现旧版 `skin = {s, p}`(旧客户端/命令行机器人)。每个接收方——房主(`public/js/host.js` 的 hello/`adoptMembers`)、成员(`public/js/main.js` 的 `joined.players`/`pjoin`)、工具机器人(`tools/host-bot.js`、`tools/member-bot.js`)——都用**同一条**校验/迁移规则在输入侧处理(v2 在范围内 → 原样;legacy `{s,p}` → `{b:1,t:s,p:p,k:1}`;其余 → `{b:1,t:0,p:0,k:1}`);房主对外二次广播(`joined`/`pjoin`)的永远是校验后的 v2 皮肤。
...
### 5.1 本地存档与重建(契约 v3 新增)

"种子 + 修改集"这一表示还带来第三个收益:**世界可以廉价落盘**。房主**和成员都**自动保存——每个进过房间的人,手里那份等价世界都会写进自己浏览器的 localStorage,这是刻意的设计点而非副作用:房间从服务器上彻底消失(迁移链断尽)后,**任何进过房的人**都能凭本地存档把它原样重建(「世界不消失」)。

- **存储格式**(由 `public/js/ui.js` 统一管理):localStorage 键 `vc-worlds` = `{ [规范化房名小写]: {room: 显示名, seed, edits: [[x,y,z,id],…], t: 最后保存时间} }`。客户端的房名规范化函数 `normRoomKey` 与服务器规则完全同款(NFC + trim + 折叠内部空白 + toLowerCase)。
- **容量上限**:最多 **8 个世界**,超出按 `t` 淘汰最旧;单个世界 `edits` 超过 **60000 条**则拒存(`putWorldSave` 变 no-op 并 `console.warn` 一次)。任何 quota/序列化失败同样 try/catch 静默降级,绝不写半截。
- **保存时机**(`public/js/main.js` 驱动):进房(`startGame`,两种模式)立即存一次;此后**每次世界变动**(自己放/挖、成员编辑回声、房主应用的成员编辑、迁移后的 `resync` 合并)触发**防抖 2 秒**的补存;退出/断线/停止游戏时同步 flush。序列化方式与 `joined.edits` 完全一致。
- **重建路径**(「进入世界」零确认链与房间选择页共用):`join` 得到 `no-room` 时查 `getWorldSave(房名)`——有存档 → 用存档的 `{seed, edits}` 建房(先把存档修改应用到新 World 再开局),世界重生;无存档 → 同名全新世界(入口链直接建,选择页则先提示"没有找到「××」")。「建立新房间」按钮例外:它**明确表示全新世界,从不查存档**,陈年存档不得劫持一个有意全新的房间。历史列表的「移除」会连同对应存档一起删除。
- **边界明确**:存档只在各玩家浏览器本地——清浏览器数据即丢档,不跨设备同步;服务器侧仍然零持久化(中继从头到尾不知道种子与修改集,见第 2 章)。
...
人物 = `{name, skin}`,其中 **`skin = {b, t, p, k}`,四个字段全是整数索引**(常量见 `public/js/constants.js`):

| 字段 | 含义 | 取值 |
|---|---|---|
| `b` | 素体,`BODIES` 索引 | 0–7,固定顺序:男矮瘦 0、男高瘦 1、男矮壮 2、男高壮 3、女矮瘦 4、女高瘦 5、女矮壮 6、女高壮 7 |
| `t` | 上衣色,`PALETTE` 索引 | 0–7(`PALETTE` 恰好 8 个十六进制颜色,沿用 v1) |
| `p` | 裤子色,`PALETTE` 索引 | 0–7 |
| `k` | 肤色,`SKIN_TONES` 索引 | 0–5(6 档由浅到深;索引 1 即 v1 的固定头色 `#e0ac69`,迁移后旧人物外观不变) |

素体只改比例,不改结构:化身仍是腿/躯干/头三段盒子,`BODIES[b].w` 缩放横向(瘦 0.82 / 壮 1.18)、`BODIES[b].h` 缩放纵向(矮 0.86 / 高 1.08),女性素体的躯干横向再乘 0.92;头部着色 `SKIN_TONES[k]`、躯干 `PALETTE[t]`、腿 `PALETTE[p]`(`renderer.addPlayer` 与菜单里的 `drawAvatar` 预览同一套比例)。人物编辑器提供名字(1–16 字)、8 素体与三组配色的完整定制;本轮**只有「人类」一个大类**(机器人等大类为规划中,配件系统整体闲置未开放)。

**校验与迁移是一条共享规则**,在所有接受外来 skin 的位置逐字一致(房主 `host.js`、成员端 `main.js`、`tools/` 机器人),按序判定:① v2 形状且全部在范围 → 原样(只取 `b,t,p,k` 四键);② legacy `{s, p}`(均为 0–7 整数)→ 迁移为 `{b:1, t:s, p:p, k:1}`;③ 其余一切 → `{b:1, t:0, p:0, k:1}`。协议上只传 `{name, skin}`;线上仍可能出现 legacy 皮肤(旧客户端),接收侧迁移、房主只二次广播校验后的 v2(见 4.2)。本地 `vc-chars` 里的旧人物在**读取时懒迁移**(同一规则),下次持久化时写回新形状。一个浏览器可建多个人物、随时切换,外观对所有玩家可见。

### 6.3 localStorage 键名(均由 `public/js/ui.js` 管理,读写包 try/catch)

| 键 | 内容 |
|---|---|
| `vc-chars` | 人物数组 `[{name, skin:{b,t,p,k}}]`;legacy `{s,p}` 条目读取时懒迁移(见 6.2) |
| `vc-active` | 活动人物的下标 |
| `vc-server` | 上次使用的服务器地址(空 = 同源) |
| `vc-history` | 房间历史 `[{room, host, t}]`,最新在前、按客户端镜像的规范化键去重、**上限 20 条**;`host` 一旦为 true 永久保持,其**语义是「我创建过这个房间」**(房间名前缀 ★ 并金色高亮),不声称「我现在是房主」——房主身份会随迁移流转,UI 从不展示实时房主归属 |
| `vc-lastroom` | 上次进入的房间显示名(「进入世界」零确认链的起点,见 5.1) |
| `vc-worlds` | 世界本地存档,格式与上限见 5.1 |
| `vc-hidden` | 在公开列表里点过「隐藏」的房名(规范化形式)数组,上限 50 条 |
| `vc-name` | v1 遗留键(曾用于预填人物名);v3 后是否保留:**待定** |

另支持 URL 参数 `?room=<房名>` 预填房间选择页的房名输入框。房名输入框留空时,客户端生成随机中文房名(形容词 + 地点 + 两位数,如"迷雾森林42",词池见 `public/js/ui.js`)并走一遍正常的"先找后建"流程:`join` 得到 `no-room` → 对这个一次性生成名跳过确认直接创建;若撞名则顺势加入。「进入世界」按钮则是完全零确认的一条链:确保有人物(没有就随机生成)→ 回上次房间 → 不在线则查本地存档重建 → 都没有则随机新房(细节见 5.1 与 `DESIGN.md` 的 Enter-world algorithm 节)。
...
- `cleanName`(字符串、trim、截 16、空补默认)与皮肤的共享 v2 校验/迁移规则(v2 范围内 → 原样;legacy `{s,p}` → 迁移;其余 → 默认皮肤,见 6.2);
...
- `pjoin`/`pmove` 的名字截断、skin 过共享 v2 校验/迁移规则(与房主侧同一条,见 6.2)、`ry` 有限性检查(NaN 会永久污染化身矩阵);重复 `joined` 不会启动第二个游戏循环;
...
- **存档只在玩家浏览器本地**(5.1)。迁移链断尽后房间从服务器消失,但任何进过房的人都能凭 `vc-worlds` 存档重建;代价是上限 8 个世界、单世界 60000 条修改,清浏览器数据即丢档、不跨设备同步,服务器侧零持久化。
...
- **存档导出与更强持久化**:本地 localStorage 存档(5.1)已落地;导出为文件、跨设备携带与服务器侧休眠/复活仍在路线图上。
