...
皮肤走**协议 v2**:`skin = {b, t, p, k}` 全整数——`b` 素体(`BODIES`,0–7)、
`t` 上衣色与 `p` 裤子色(`PALETTE` 8 色索引,0–7)、`k` 肤色(`SKIN_TONES`,0–5),
常量见 `public/js/constants.js`。**legacy 兼容**:线上仍可能出现旧版 `{s, p}`
(0–7 整数对,旧客户端/机器人),接收方一律在输入侧迁移为 `{b:1, t:s, p:p, k:1}`;
其余畸形坍缩为 `{b:1, t:0, p:0, k:1}`;房主对外广播的永远是校验后的 v2 皮肤。
...
本节 JSON 为**字段示例,结构与实抓一致**;`skin` 已按皮肤协议 v2 更新,未重新抓包。)
...
```json
{"t":"msg","d":{"t":"hello","name":"小树","skin":{"b":2,"t":3,"p":0,"k":4}}}
```

中继 → 房主 A(盖上 `from` 后转发):

```json
{"t":"msg","from":5,"d":{"t":"hello","name":"小树","skin":{"b":2,"t":3,"p":0,"k":4}}}
```

房主 A → 中继(单播 `joined` 给新人):

```json
{"t":"msg","to":5,"d":{"t":"joined","room":"石榴小镇","seed":12345,"id":5,"players":[{"id":1,"name":"阿石","p":[8.5,33,8.5],"ry":0,"skin":{"b":1,"t":1,"p":5,"k":1}},{"id":4,"name":"小梅","p":[8,40,8],"ry":0,"skin":{"b":5,"t":2,"p":6,"k":3}}],"edits":[]}}
```

中继 → 新成员 M2(去掉 `to` 信封,只剩 `d`):

```json
{"t":"msg","d":{"t":"joined","room":"石榴小镇","seed":12345,"id":5,"players":[{"id":1,"name":"阿石","p":[8.5,33,8.5],"ry":0,"skin":{"b":1,"t":1,"p":5,"k":1}},{"id":4,"name":"小梅","p":[8,40,8],"ry":0,"skin":{"b":5,"t":2,"p":6,"k":3}}],"edits":[]}}
```
...
```json
{"t":"cast","d":{"t":"pjoin","id":5,"name":"小树","p":[8,40,8],"ry":0,"skin":{"b":2,"t":3,"p":0,"k":4}},"except":5}
```

中继 → 老成员 M1:

```json
{"t":"msg","d":{"t":"pjoin","id":5,"name":"小树","p":[8,40,8],"ry":0,"skin":{"b":2,"t":3,"p":0,"k":4}}}
```
...
- **房主端清洗一切成员输入**(`public/js/host.js`):
  名字 trim 后截到 16 字符、空名回退为 `玩家`;皮肤走共享 v2 校验/迁移规则——
  v2 形状全整数且在范围(`b,t,p` 0–7、`k` 0–5)→ 原样(只取四键);
  legacy `{s,p}`(0–7 整数)→ `{b:1,t:s,p:p,k:1}`;其余 → `{b:1,t:0,p:0,k:1}`。
  同一条规则也落在成员端(`public/js/main.js`)与命令行机器人(`tools/`)。
